## Plan: 3D IMU Attitude Dog Icon
 based on my question: can you plan implmenting a 3D animation with Freenove Robo Dog Icon based on IMU status ?
...
1. Confirm current telemetry loop in [mtDogLogicMixin.py](mtDogLogicMixin.py) (`poll_telemetry`, `command_receiver_worker`, `handle_cmd_line`).
...
4. Create a new widget class `Attitude3DWidget(QOpenGLWidget)` in [ui/attitude_3d_widget.py](ui/attitude_3d_widget.py) rendering a textured “dog icon” quad.
5. Embed the widget into the right control panel layout (via [ui/control_panel_sections.py](ui/control_panel_sections.py) / [ui/ui_client.py](ui/ui_client.py)) and update it on an existing QTimer tick.
6. Add UI polish + robustness: smoothing, degree clamping, “stale IMU” dimming, and a disable toggle in settings.

### Further Considerations 1–3, 5–25 words each
1. Asset choice: reuse an existing image from [Picture/logo_Mini.png](Picture/logo_Mini.png) or add a dedicated transparent PNG for the dog.
