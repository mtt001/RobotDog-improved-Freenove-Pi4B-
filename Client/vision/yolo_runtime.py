...
===============================================================================
 ...
 v1.03  (2026-02-02)          : Dual YOLO status text formatting
     • Add rich-text title + ranked detection list with color highlights.
     • Provide target summary line aligned with dual-model detections.
 v1.04  (2026-02-02)          : Compare shows current confidence
     • Display mt_ball and sports-ball confidence thresholds in Compare titles.
 v1.02  (2026-02-01)          : Ball-priority tracking + compare counters
     • Ball always wins target selection when present; dog is second priority.
     • Fix compare panes under dual mode (use raw frame, correct per-model dets).
     • Add running detect counters (mt_Ball / Sport_Ball / Total).
 v1.01  (2026-02-01)          : Dual-model YOLO runtime (COCO + mt_ball)
     • Run COCO yolov8n.pt + custom best.pt each inference cycle (FPS-capped).
     • Select stable target across mt_ball/sports ball/dog/person.
     • Compute ball-only HSV 5/95 percentile thresholds and mask.
     • Populate host._vision_status_text for bottom message pane.
...
class YoloRuntimeController:
    def __init__(self, host, *, compare_window_factory=None, train_hist_window_factory=None):
        ...
        host._yolo_compare_hit_best = 0
        host._yolo_compare_hit_orig = 0
        host._yolo_compare_hit_total = 0
        ...
        # Dual-model YOLO (COCO + mt_ball)
        host._yolo_dual_coco_all = []
        host._yolo_dual_coco_display = []
        host._yolo_dual_mt = []
        host._yolo_dual_last_run_t = 0.0  # perf_counter()
        host._yolo_dual_last_total_ms = 0.0
        host._yolo_dual_last_coco_ms = 0.0
        host._yolo_dual_last_mt_ms = 0.0
        host._vision_target = None
        host._vision_status_text = ""
        host._vision_refine_mask = None
        host._vision_refine_hsv = None
        host._vision_target_lock_left = 0
        host._vision_target_conf = 0.0
        host._vision_target_source = ""
        host._vision_target_class = ""
        host._vision_target_bbox = None
        ...
        # Dedicated detectors for dual-model mode
        host.yolo_detector_coco = (
            YOLOBallDetector(model_path=host.yolo_model_orig_path, ball_class_id=32) if YOLOBallDetector is not None else None
        )
        host.yolo_detector_mt = (
            YOLOBallDetector(model_path=host.yolo_model_best_path, ball_class_id=0) if YOLOBallDetector is not None else None
        )
    ...
    def _compact_label(label: str) -> str:
        ...
        if s in ("mt_ball", "mt ball", "mtball"):
            return "mt_Ball"
        return (label or "Obj").strip().replace(" ", "_")

    def _dual_should_run_now(self) -> bool:
        host = self._host
        try:
            cap_fps = float(getattr(host, "yolo_dual_cap_fps", 0.0) or 0.0)
        except Exception:
            cap_fps = 0.0
        if cap_fps <= 0:
            return True
        min_dt = 1.0 / max(0.1, cap_fps)
        now_t = time.perf_counter()
        last_t = float(getattr(host, "_yolo_dual_last_run_t", 0.0) or 0.0)
        return (now_t - last_t) >= min_dt

    def _dual_refine_ball_mask(self, frame_bgr, bbox_xyxy) -> None:
        """Ball-only refinement per spec: HSV 5th/95th percentiles within ROI."""
        host = self._host
        host._vision_refine_mask = None
        host._vision_refine_hsv = None
        if frame_bgr is None or getattr(frame_bgr, "size", 0) == 0 or bbox_xyxy is None:
            return
        ...
        try:
            ...
            lo_hsv = tuple(int(max(0, min(255, round(v)))) for v in lo.tolist())
            hi_hsv = tuple(int(max(0, min(255, round(v)))) for v in hi.tolist())
        except Exception:
            return
        try:
            lo_arr = np.array(lo_hsv, dtype=np.uint8)
            hi_arr = np.array(hi_hsv, dtype=np.uint8)
            roi_mask = cv2.inRange(hsv, lo_arr, hi_arr)
            full = np.zeros((h0, w0), dtype=np.uint8)
            full[y1:y2, x1:x2] = roi_mask
            host._vision_refine_mask = full
            host._vision_refine_hsv = (lo_hsv, hi_hsv)
        except Exception:
            host._vision_refine_mask = None
            host._vision_refine_hsv = None

    def _dual_select_target(self, candidates, *, stickiness_frames: int = 15, switch_margin: float = 0.15):
        """Select a stable target per COPILOT_VISION_SPEC.md.

        User override: Ball is always top priority. Dog is second priority.
        """
        host = self._host
        prev_bbox = getattr(host, "_vision_target_bbox", None)
        prev_class = str(getattr(host, "_vision_target_class", "") or "")
        prev_source = str(getattr(host, "_vision_target_source", "") or "")
        lock_left = int(getattr(host, "_vision_target_lock_left", 0) or 0)

        def _priority(cls_name: str) -> int:
            s = str(cls_name or "").strip().lower()
            if s in ("mt_ball", "sports ball"):
                return 0
            if s == "dog":
                return 1
            if s == "person":
        ...
        for c in candidates:
            try:
                p = _priority(c.get("class"))
                if best is None or p < best_p or (p == best_p and float(c["conf"]) > float(best["conf"])):
                    best = c
                    best_p = int(p)
            except Exception:
                continue
        ...
        if best is not None:
            try:
                best_group = [c for c in candidates if _priority(c.get("class")) == best_p]
            except Exception:
                best_group = []
        best_in_group = None
        for c in best_group:
            try:
                if best_in_group is None or float(c["conf"]) > float(best_in_group["conf"]):
                    best_in_group = c
            except Exception:
                continue

        prev_match = None
        if prev_bbox is not None and prev_class and prev_source:
            for c in candidates:
                try:
                    if c.get("class") != prev_class or c.get("source") != prev_source:
                        continue
                    if self._bbox_iou(prev_bbox, c.get("bbox")) >= 0.30:
                        prev_match = c
                        break
                except Exception:
                    continue

        # Priority rule: if any ball exists, we only consider balls.
        prev_p = _priority(prev_class)

        if best_in_group is None:
            chosen = None
            new_lock = 0
        else:
            # If we're leaving a higher-priority target (e.g. dog) because a ball appeared,
            # switch immediately.
            if best_p < prev_p:
                chosen = best_in_group
                new_lock = int(stickiness_frames)
            else:
                # Same priority band: apply stickiness (stay on previous if it still matches).
                if prev_match is None or _priority(prev_match.get("class")) != best_p:
                    chosen = best_in_group
                    new_lock = int(stickiness_frames)
                else:
                    chosen = prev_match
                    new_lock = max(0, lock_left - 1)
                    if new_lock <= 0:
                        try:
                            if float(best_in_group["conf"]) >= (float(prev_match["conf"]) + float(switch_margin)):
                                chosen = best_in_group
                                new_lock = int(stickiness_frames)
                        except Exception:
                            pass
        ...
        host._vision_target = chosen
        host._vision_target_lock_left = int(new_lock)
        host._vision_target_conf = float(chosen.get("conf", 0.0) or 0.0)
        host._vision_target_source = str(chosen.get("source", "") or "")
        host._vision_target_class = str(chosen.get("class", "") or "")
        host._vision_target_bbox = chosen.get("bbox")
        return chosen

    def _dual_update_status_text(self, *, coco_count: int, coco_display_count: int, mt_count: int) -> None:
        host = self._host
        try:
            cap_fps = float(getattr(host, "yolo_dual_cap_fps", 0.0) or 0.0)
        except Exception:
            cap_fps = 0.0
        coco_ms = float(getattr(host, "_yolo_dual_last_coco_ms", 0.0) or 0.0)
        mt_ms = float(getattr(host, "_yolo_dual_last_mt_ms", 0.0) or 0.0)
        total_ms = float(getattr(host, "_yolo_dual_last_total_ms", 0.0) or 0.0)

        coco_fps = (1000.0 / coco_ms) if coco_ms > 0 else 0.0
        mt_fps = (1000.0 / mt_ms) if mt_ms > 0 else 0.0
        total_fps = (1000.0 / total_ms) if total_ms > 0 else 0.0

        def _fmt_label(label: str) -> str:
            base = str(label or "").replace("_", " ").strip().lower()
            if base in ("mt ball", "mt_ball", "mt-ball"):
                return "mt_Ball"
            if base in ("sports ball", "sport ball", "sport_ball"):
                return "sport_ball"
        ...
        items = []
        for b in list(getattr(host, "_yolo_dual_mt", []) or []):
            try:
                conf = float(getattr(b, "conf", 0.0) or 0.0)
            except Exception:
                conf = 0.0
            try:
                label = str(getattr(b, "label", "mt_ball") or "mt_ball")
            except Exception:
                label = "mt_ball"
            items.append({"label": label, "conf": conf, "source": "MT"})

        for b in list(getattr(host, "_yolo_dual_coco_display", []) or []):
            try:
                conf = float(getattr(b, "conf", 0.0) or 0.0)
            except Exception:
                conf = 0.0
            ...
            if not label:
                ...
                if ci == 32:
                    ...
                    label = f"cls{ci}" if ci >= 0 else "object"
            items.append({"label": label, "conf": conf, "source": "COCO"})

        items.sort(key=lambda it: float(it.get("conf", 0.0) or 0.0), reverse=True)

        def _color_for(item_label: str, source: str) -> str:
            if str(source or "").upper() == "MT":
                return "#00c853"
            if _fmt_label(item_label).lower() == "sport_ball":
                return "#ffd54f"
            return "#cfd6df"
        ...
        if items:
            det_parts = []
            for it in items:
                lbl = _fmt_label(it.get("label", ""))
                conf = float(it.get("conf", 0.0) or 0.0)
                color = _color_for(it.get("label", ""), it.get("source", ""))
                det_parts.append(_span(f"{lbl} {conf:.2f}", color))
            det_line = "Detections: " + ", ".join(det_parts)
        else:
            det_line = "Detections: --"

        tgt = getattr(host, "_vision_target", None)
        if isinstance(tgt, dict):
            t_cls = _fmt_label(str(tgt.get("class", "") or ""))
            t_src = str(tgt.get("source", "") or "")
            try:
                t_cf = float(tgt.get("conf", 0.0) or 0.0)
            except Exception:
                t_cf = 0.0
            tgt_color = _color_for(t_cls, t_src)
            tgt_parts = [_span(f"{t_cls} {t_cf:.2f} ({t_src})", tgt_color)]
            added = 0
            for it in items:
                if added >= 2:
                    break
                ...
                if lbl == t_cls and src == t_src and abs(conf - t_cf) < 1e-6:
                    continue
                color = _color_for(it.get("label", ""), src)
                tgt_parts.append(_span(f"{lbl} {conf:.2f}", color))
                added += 1
            tgt_line = "Target: " + ", ".join(tgt_parts)
        else:
            tgt_line = "Target: --"

        refine_line = "Refine: --"
        hsv = getattr(host, "_vision_refine_hsv", None)
        if isinstance(hsv, (tuple, list)) and len(hsv) == 2:
            try:
                lo, hi = hsv
                refine_line = f"Refine HSV: lo{tuple(int(v) for v in lo)} hi{tuple(int(v) for v in hi)}"
            except Exception:
                refine_line = "Refine: --"

        cap_line = (
            f"Dual YOLO detection (cap {cap_fps:.1f}fps) :  MT (best.pt) ,  Yolo (yolov8n.pt)"
            if cap_fps > 0
            else "Dual YOLO detection :  MT (best.pt) ,  Yolo (yolov8n.pt)"
        )
        perf_line = (
            f"COCO {coco_ms:.1f}ms ({coco_fps:.0f}fps) | MT {mt_ms:.1f}ms ({mt_fps:.0f}fps) | total {total_ms:.1f}ms ({total_fps:.0f}fps)"
        )
        count_line = f"Counts: COCO {coco_display_count}/{coco_count}, MT {mt_count}"
        host._vision_status_text = "<br>".join([cap_line, perf_line, det_line, tgt_line, count_line, refine_line])

    def _run_dual_inference(self, frame_bgr):
        host = self._host
        if frame_bgr is None or getattr(frame_bgr, "size", 0) == 0:
            ...
            host._vision_refine_mask = None
            host._vision_refine_hsv = None
            host._vision_status_text = "Dual YOLO: empty frame"
            return

        if not self._dual_should_run_now():
            return

        t_total0 = time.perf_counter()
        host._yolo_dual_last_run_t = t_total0

        coco = getattr(host, "yolo_detector_coco", None)
        mt = getattr(host, "yolo_detector_mt", None)
        if coco is None or mt is None:
            host._vision_status_text = "Dual YOLO: not available (ultralytics/weights missing)"
            host._yolo_dual_coco_all = []
            host._yolo_dual_coco_display = []
            host._yolo_dual_mt = []
            host._vision_target = None
        ...
        try:
            coco_conf = float(getattr(host, "yolo_coco_conf", 0.1) or 0.1)
        except Exception:
            coco_conf = 0.1
        try:
            mt_conf = float(getattr(host, "yolo_mt_ball_conf", 0.1) or 0.1)
        except Exception:
            mt_conf = 0.1
        ...
        try:
            coco_all = coco.analyze_classes(frame_bgr, classes=None, conf=coco_conf, max_det=30)
            coco_err = str(getattr(coco, "last_error", "") or "").strip()
        except Exception as e:
            coco_all = []
            coco_err = f"COCO exception: {e}"
        t1 = time.perf_counter()
        host._yolo_dual_last_coco_ms = (t1 - t0) * 1000.0

        mt_boxes = []
        mt_err = ""
        t2 = time.perf_counter()
        try:
            mt_boxes = mt.analyze_classes(frame_bgr, classes=[0], conf=mt_conf, max_det=10)
            mt_err = str(getattr(mt, "last_error", "") or "").strip()
        except Exception as e:
            mt_boxes = []
            mt_err = f"MT exception: {e}"
        t3 = time.perf_counter()
        host._yolo_dual_last_mt_ms = (t3 - t2) * 1000.0

        host._yolo_dual_last_total_ms = (time.perf_counter() - t_total0) * 1000.0

        # COCO display policy: show interest classes if present, else show all detections.
        interest_ids = {0, 16, 32}
        coco_interest = [b for b in (coco_all or []) if int(getattr(b, "cls", -1) or -1) in interest_ids]
        coco_display = coco_interest if coco_interest else list(coco_all or [])

        # Normalize mt_ball label for display.
        for b in mt_boxes or []:
            try:
                b.label = "mt_ball"
            except Exception:
                pass

        host._yolo_dual_coco_all = list(coco_all or [])
        host._yolo_dual_coco_display = list(coco_display or [])
        host._yolo_dual_mt = list(mt_boxes or [])

        # Compare view (mt best.pt vs COCO sports ball) under dual mode.
        try:
            sports_ball_boxes = [b for b in (coco_all or []) if int(getattr(b, "cls", -1) or -1) == 32]
        except Exception:
            sports_ball_boxes = []
        for b in sports_ball_boxes or []:
            try:
                b.label = "sports ball"
            except Exception:
                pass
        host._yolo_compare_detections_best = list(mt_boxes or [])
        host._yolo_compare_detections_orig = list(sports_ball_boxes or [])

        # Running hit counters (increment when either model detects a ball).
        if bool(getattr(host, "yolo_compare_enabled", False)):
            best_hit = bool(mt_boxes)
            orig_hit = bool(sports_ball_boxes)
            if best_hit:
                host._yolo_compare_hit_best = int(getattr(host, "_yolo_compare_hit_best", 0) or 0) + 1
            if orig_hit:
                host._yolo_compare_hit_orig = int(getattr(host, "_yolo_compare_hit_orig", 0) or 0) + 1
            if best_hit or orig_hit:
                host._yolo_compare_hit_total = int(getattr(host, "_yolo_compare_hit_total", 0) or 0) + 1

        # Candidates for target selection (strict set).
        candidates = []
        for b in mt_boxes or []:
            try:
                candidates.append(
                    {
                        ...
                        "conf": float(getattr(b, "conf", 0.0) or 0.0),
                        "bbox": (
                            float(getattr(b, "x1", 0.0)),
                            float(getattr(b, "y1", 0.0)),
                            float(getattr(b, "x2", 0.0)),
                            float(getattr(b, "y2", 0.0)),
                        ),
                        "source": "MT",
                        "det": b,
                    }
        ...
        chosen = self._dual_select_target(candidates, stickiness_frames=15, switch_margin=0.15)

        # Ball-only refinement
        if isinstance(chosen, dict):
            cls_name = str(chosen.get("class", "") or "")
            if cls_name in ("mt_ball", "sports ball"):
                self._dual_refine_ball_mask(frame_bgr, chosen.get("bbox"))
            else:
                host._vision_refine_mask = None
                host._vision_refine_hsv = None
        else:
        ...
        self._dual_update_status_text(
            coco_count=int(len(coco_all or [])),
            coco_display_count=int(len(coco_display or [])),
            mt_count=int(len(mt_boxes or [])),
        )
        if (not coco_all) and coco_err:
            host._vision_status_text = (host._vision_status_text + "<br>" + f"COCO err: {coco_err}").strip()
        if (not mt_boxes) and mt_err:
            host._vision_status_text = (host._vision_status_text + "<br>" + f"MT err: {mt_err}").strip()
    ...
    def update_compare_view(self, frame_bgr):
        ...
        # Titles + totals (running hit counters).
        try:
            b = int(getattr(host, "_yolo_compare_hit_best", 0) or 0)
            o = int(getattr(host, "_yolo_compare_hit_orig", 0) or 0)
            t = int(getattr(host, "_yolo_compare_hit_total", 0) or 0)
            bp = (100.0 * b / t) if t > 0 else 0.0
            op = (100.0 * o / t) if t > 0 else 0.0
            if host.yolo_compare_window is not None:
                mt_conf = float(getattr(host, "yolo_mt_ball_conf", 0.0) or 0.0)
                coco_conf = float(getattr(host, "yolo_coco_conf", 0.0) or 0.0)
                left_title = f"mt_Ball #{b} ({bp:.1f}%)  conf:{mt_conf:.2f}"
                right_title = f"Sport_Ball #{o} ({op:.1f}%)  conf:{coco_conf:.2f}"
                host.yolo_compare_window.set_titles(left_title, right_title)
                try:
                    host.yolo_compare_window.set_total_text(f"Total Detect Count: {t} (100.0%)")
                except Exception:
                    pass
        except Exception:
            pass
        ...
        try:
            if host._yolo_compare_detections_best:
                host.overlay.draw_yolo_dual_detections(left, [], host._yolo_compare_detections_best, active_target=None)
        except Exception:
            pass
        try:
            if host._yolo_compare_detections_orig:
                host.overlay.draw_yolo_dual_detections(right, host._yolo_compare_detections_orig, [], active_target=None)
        except Exception:
            pass
        try:
    ...
    def run_ball_mode(self, frame_bgr):
        host = self._host
        if bool(getattr(host, "yolo_dual_enabled", False)):
            self._run_dual_inference(frame_bgr)
            try:
                host.overlay.draw_yolo_dual_detections(
                    frame_bgr,
                    list(getattr(host, "_yolo_dual_coco_display", []) or []),
                    list(getattr(host, "_yolo_dual_mt", []) or []),
                    active_target=getattr(host, "_vision_target", None),
                )
            except Exception:
                pass
            ...
            if isinstance(tgt, dict):
                try:
                    x1, y1, x2, y2 = tgt.get("bbox")
                    cx = int(round(0.5 * (float(x1) + float(x2))))
                    cy = int(round(0.5 * (float(y1) + float(y2))))
                    rr = 0.5 * max(6.0, min(abs(float(x2) - float(x1)), abs(float(y2) - float(y1))))
                except Exception:
                    cx = cy = 0
                    rr = 0.0
                ...
                try:
                    host._ball_locked = bool(
                        ball_center is not None and int(getattr(host.ball_tracker, "missed_frames", 0) or 0) == 0
                    )
    ...
    def run_test_mode(self, frame_bgr):
        ...
        if bool(getattr(host, "yolo_dual_enabled", False)):
            ...
            try:
                ...
                pass
            try:
                # Compare should start from a clean (pre-overlay) frame, otherwise both panes look identical.
                src_cmp = host.last_display_frame_bgr if host.last_display_frame_bgr is not None else frame_bgr
                self.update_compare_view(src_cmp)
            except Exception:
                pass
