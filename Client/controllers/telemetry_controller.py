...
===============================================================================
 ...
 v1.01  (2026-02-02 20:31)    : Add IMU telemetry fields + polling timer.
...
class TelemetryController:
    def __init__(self, host, *, low_voltage_threshold: float):
        ...
        # IMU attitude telemetry (roll/pitch/yaw)
        host.imu_roll = 0.0
        host.imu_pitch = 0.0
        host.imu_yaw = 0.0
        host.imu_last_ts = 0.0
        host.imu_hz = 0.0
        host.imu_poll_hz = 2.0
        host.imu_smoothing_alpha = 0.35
        host.imu_timer = QTimer()
    ...
    def start(self):
        ...
        # IMU polling (separate cadence to avoid spamming other telemetry).
        try:
            interval_ms = int(round(1000.0 / float(host.imu_poll_hz)))
        except Exception:
            interval_ms = 500
        interval_ms = max(200, min(2000, interval_ms))
        try:
            host.imu_timer.timeout.connect(host.poll_imu)
            host.imu_timer.start(interval_ms)
        except Exception:
            pass
