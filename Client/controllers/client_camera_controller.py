...
===============================================================================
 ...
 v1.02  (2026-02-02)          : Default MacBook camera
     • Prefer Index 1 (MacBook) by default and update label casing.
 v1.01  (2026-02-01)          : Friendlier camera labels
     • Display "Index 0 (iPhone)" and "Index 1 (Macbook)" in bottom camera selector.
...
class ClientCameraController:
    def __init__(self, host):
        ...
        if 1 in host._client_cam_indices:
            host._client_cam_indices = [1] + [i for i in host._client_cam_indices if i != 1]
    ...
    def refresh_client_camera_list(self, *, select_index=None):
        ...
        for idx in available:
            # User convention for this project (macOS Continuity camera setup)
            if int(idx) == 0:
                label = "Index 0 (iPhone)"
            elif int(idx) == 1:
                label = "Index 1 (MacBook)"
            else:
                label = f"Index {idx}"
        ...
        if not available:
            ...
            if select_index is None:
                if 1 in host._client_cam_combo_map:
                    select_index = 1
                else:
