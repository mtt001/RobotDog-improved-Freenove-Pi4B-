===============================================================================
 ...
 v1.33  (2026-02-02)          : Expand file hierarchy + GitHub backup guide
     • Restore the original file-tree style with per-file descriptions.
     • Include non-Python project assets (datasets, models, VS Code config, etc.).
     • Add GitHub backup / restore workflow notes.
 v1.32  (2026-02-02)          : Full Python file hierarchy
     • List all `.py` files in the Client folder as a tree.
...
├── .github/                               # GitHub/Copilot project automation
│   ├── copilot-instructions.md            # Copilot rules for this workspace
│   ├── copilot-instructions.md.debug      # Debug copy (optional)
│   └── copilot-instructions.md.tooBig     # Scratch copy (optional)
├── .vscode/                               # VS Code workspace config
│   ├── settings.json                      # Editor settings for this repo
│   └── tasks.json                         # One-click run/train tasks
├── __pycache__/                           # Python bytecode cache (safe to delete)
├── .gitignore                             # Git ignore rules (caches, runs, etc.)
├── README.md                              # This file
├── README.Development Guidelines.md       # Development notes & conventions
├── README.Main File Structure.md          # Main window / app structure notes
├── CODE_HIERARCHY.txt                     # High-level code map
├── COPILOT_VISION_SPEC.md                 # Vision/overlay/spec notes used for refactor
├── AI_YOLO_Datasets.md                    # Dataset notes & YOLO training guidance
├── smartdog.md                            # Convenience alias / run notes
├── mtDog.md                               # Dog behavior/modes notes
├── mtVisionAI.md                          # AI vision notes
├── mtSysPlanning.md                       # System planning notes
├── FullTrackingBall.md                    # Ball tracking notes
├── mtDogBallTrackExplain.md               # Ball tracking explanation
├── mtDogVisionAI_YOLO_OpenCV_Hybrid.md    # Hybrid vision notes
├── mtDogVisionAI_YOLO_OpenCV_Hybrid v1,2.md # Older hybrid vision notes
├── mtBallDetectCV.md                      # CV detector notes
├── Handoff Note,CODEX.md                  # Dev handoff notes / change log
├── Tutorial.pdf                           # Vendor tutorial/reference
├── TestImageAI.png                        # Test image (AI)
├── TestImageAI-1.png                      # Test image (AI variant)
├── TestImageBallFloor.png                 # Test image (ball on floor)
├── TestImageBallFloor_cv_overlay.png      # Generated CV overlay (floor)
├── TestImageBallGlass.png                 # Test image (ball on glass)
├── TestImageBallGlass_cv_overlay.png      # Generated CV overlay (glass)
├── IP.txt                                 # Robot IP address (target)
├── point.txt                              # Calibration/point note
├── mtDogMain.log                          # Runtime log output (generated)
├── mtBallDetectCV.log                     # Detector log output (generated)
├── yolov8n.pt                             # COCO YOLO model weights (Ultralytics)
├── runs/                                  # Ultralytics run outputs (training/detect)
│   └── detect/
│       └── train5/                        # Example training run output
│           └── weights/
│               ├── best.pt                # Trained weights (custom)
│               └── last.pt                # Last-epoch weights (custom)
├── AI_datasets/                           # YOLO datasets (can be large)
│   ├── yolo_ball_v01/                     # Dataset v01 (ball)
│   │   ├── dataset.yaml                   # Ultralytics dataset config
│   │   ├── README.md                      # Dataset notes
│   │   ├── images/                        # Images (train/val)
│   │   ├── labels/                        # YOLO labels (train/val)
│   │   └── meta/                          # Per-image metadata (train/val/*.json)
│   └── yolo_ball_v01拷貝/                  # Dataset copy (backup)
│       ├── dataset.yaml                   # Ultralytics dataset config
│       ├── README.md                      # Dataset notes
│       ├── images/                        # Images (train/val)
│       ├── labels/                        # YOLO labels (train/val)
│       └── meta/                          # Per-image metadata (train/val/*.json)
├── Picture/                               # UI assets/images
│   ├── dog_client.png                     # Splash image
│   ├── dog_calibration.png                # Calibration UI image
│   └── logo_Mini.png                      # App icon
├── Face/                                  # Face recognition assets
│   ├── face.yml                           # Face labels/training config
│   ├── haarcascade_frontalface_default.xml # Haar cascade for face detect
│   ├── name.txt                           # Known names list
│   └── 0.jpg                              # Sample face image
├── Temp/                                  # Temporary scripts + generated artifacts
│   ├── fix_v01_split.py                   # Dataset split helper
│   └── ai_yolo_compare/                   # AI/YOLO compare images/overlays
├── __init__.py                            # Package marker (workspace)
├── mtDogMain.py                           # Main PyQt5 GUI entry point
├── mtDogLogicMixin.py                     # Dog control + networking mixin for main window
├── mtDogBallTrack.py                      # Ball tracking state + smoothing logic
├── MT_BallTracking.py                     # Legacy/experimental ball tracking module
├── Main.py                                # Legacy GUI entry (pre-refactor)
├── Thread.py                              # Legacy threading helpers
├── Face.py                                # Legacy face helper module
├── config/                                # Configuration and calibration
│   ├── __init__.py                        # Package marker
│   ├── mtDogConfig.py                     # Central app settings / defaults
│   ├── balltrack_calib.json               # Ball tracking calibration file
│   └── mtBall_Calib.json                  # CV calibration data
├── controllers/                           # Refactored controller modules
│   ├── __init__.py                        # Package marker
│   ├── frame_update_controller.py         # Per-frame capture/detect/render loop
│   ├── client_camera_controller.py        # Camera enumeration/selection logic
│   ├── server_reconnect_controller.py     # Keep-alive + reconnect behavior
│   ├── telemetry_controller.py            # Telemetry parsing + UI state updates
│   ├── dog_command_controller.py          # Command constants + send helpers
│   ├── ball_tracking_controller.py        # Ball tracking state machine + actions
│   └── pid_controller.py                  # PID controller used in tracking
├── ui/                                    # UI widgets, handlers, debug windows
│   ├── __init__.py                        # Package marker
│   ├── ui_client.py                       # Main window UI layout
│   ├── ui_event_handlers.py               # UI event wiring (buttons/keys)
│   ├── control_panel_sections.py          # Control panel widgets/sections
│   ├── status_ui_controller.py            # Bottom message pane controller
│   ├── status_overlay_controller.py       # Overlay status composition (no on-frame text)
│   ├── common_widgets.py                  # Shared UI widget helpers
│   ├── yolo_debug_windows.py              # YOLO debug/compare windows
│   ├── cv_debug_windows.py                # CV debug windows
│   ├── ai_hist_windows.py                 # Histogram/diagnostic windows
│   ├── ui_face.py                         # Face window UI
│   └── ui_led.py                          # LED window UI
├── vision/                                # Vision pipelines (YOLO/CV/AI)
│   ├── __init__.py                        # Package marker
│   ├── yolo_runtime.py                    # Dual-model YOLO runtime + target selection
│   ├── ai_vision_controller.py            # GPT Vision controller (cloud)
│   ├── mask_picker.py                     # Interactive mask picker window
│   ├── cv_hist_debug.py                   # CV histogram + debug utilities
│   ├── utils/
│   │   ├── __init__.py                    # Package marker
│   │   ├── overlay_renderer.py            # Draw boxes/labels for detections
│   │   └── motion_grid_builder.py         # Motion/action grid layout builder
│   └── legacy/                            # Legacy detectors kept for reference
│       ├── __init__.py
│       ├── cv_ball_detection.py           # Legacy CV ball detection pipeline
│       ├── mtBallDetectCV.py              # Legacy CV ball detector
│       ├── mtBallDetectYOLO.py            # Legacy YOLO wrapper
│       └── mtBallDetectAI.py              # Legacy GPT Vision wrapper
├── docs/                                  # Calibration docs/tools
│   └── calibration/
│       ├── calibrateServo.md              # Calibration instructions
│       └── Calibration.py                 # Calibration helper script/UI
├── tools/                                 # Utility scripts and experiments
│   ├── __init__.py                        # Package marker
│   ├── testVideoStream.py                 # Video stream debug tool
│   ├── testServo.py                       # Servo test tool
│   ├── testHeadMoving.py                  # Head motion test
│   ├── testBallSearchAI.py                # AI ball search test
│   ├── testAIVisionRepeat.py              # GPT Vision stability test
│   ├── testImageBall_CVexpert.py          # CV overlay test
│   ├── testImageBall_GPT_yolo.py          # GPT vs YOLO compare test
│   ├── calibrateServo.py                  # Servo calibration helper
│   ├── Client.original.py                 # Original Client snapshot
│   ├── mtDogMain.original.py              # Original mtDogMain snapshot
│   └── yolo/
│       ├── __init__.py                    # Package marker
│       └── yolo_labeling.py               # YOLO labeling helper
└── legacy/                                # Legacy app snapshots (kept for reference)
    ├── __init__.py                        # Package marker
    ├── Client.py                          # Legacy TCP/video client
    ├── ClientV102.py                      # Older legacy client variant
    └── Main.original.py                   # Original GUI snapshot
```
...
├── controllers/
│   ├── __init__.py
│   ├── ball_tracking_controller.py
│   ├── client_camera_controller.py
│   ├── dog_command_controller.py
│   ├── frame_update_controller.py
│   ├── pid_controller.py
│   ├── server_reconnect_controller.py
...
│   ├── Client.original.py
│   ├── calibrateServo.py
│   ├── mtDogMain.original.py
│   ├── testAIVisionRepeat.py
│   ├── testBallSearchAI.py
│   ├── testHeadMoving.py
│   ├── testImageBall_CVexpert.py
│   ├── testImageBall_GPT_yolo.py
│   ├── testServo.py
│   ├── testVideoStream.py
│   └── yolo/
│       ├── __init__.py
...
│   ├── ai_hist_windows.py
│   ├── common_widgets.py
│   ├── control_panel_sections.py
│   ├── cv_debug_windows.py
│   ├── status_overlay_controller.py
│   ├── status_ui_controller.py
│   ├── ui_client.py
│   ├── ui_event_handlers.py
│   ├── ui_face.py
│   ├── ui_led.py
│   └── yolo_debug_windows.py
└── vision/
```

---

## ☁️ GitHub Backup & Handling (What To Do)

This repo already has a local `.git/` folder; the steps below describe a safe workflow to use GitHub as a reliable backup, while avoiding common pitfalls (large datasets, trained weights, secrets).

### 1) First-time setup (one machine)

- Create a new GitHub repository (private recommended).
- Add it as a remote and push your current branch:
...
### 2) Daily backup workflow (recommended)
...
Tips:
- Commit small, focused changes (UI, vision, docs) rather than huge “everything” commits.
- Use branches for risky experiments: `git checkout -b exp/yolo-tuning`.

### 3) Big files: datasets, runs, weights

The following can grow very large:
- `AI_datasets/` (images/labels)
- `runs/` (training outputs)
- `*.pt` weights (e.g., `runs/detect/.../weights/best.pt`)

Options:
- **Option A (common)**: keep these out of git and back them up separately (external drive / cloud).
- **Option B**: use **Git LFS** for weights and (optionally) datasets.

If you use Git LFS for model weights:
...
git add yolov8n.pt runs/detect/*/weights/*.pt
git commit -m "Track model weights with Git LFS"
git push
```

### 4) Secrets & local-only settings (critical)

- Never commit `OPENAI_API_KEY` (keep it as an environment variable).
- Decide whether `IP.txt` should be committed:
    - If it contains private IPs, prefer `IP.example.txt` in git and keep `IP.txt` local.
- Don’t commit caches/logs: `__pycache__/`, `*.log`, `.DS_Store`.
...
### 6) “Known good” snapshots

- Tag stable states before big refactors/training changes:

```
git tag -a v3.25-client -m "Known good client"
git push --tags
```

- For large artifacts (trained weights / sample videos), consider **GitHub Releases** instead of commits.

---

## 📁 File Hierarchy

See the **File Hierarchy** section earlier in this README (kept as the single source of truth).
...
**Key Features**:
- **Color Picker**: Qt color dialog with live preview
- **RGB/HL Modes**: Dual color space input (sliders + text fields)
- **Animation Modes**: Static, breathing, rainbow, chase effects
- **Brightness Control**: 0-100% dimming via dial widget
