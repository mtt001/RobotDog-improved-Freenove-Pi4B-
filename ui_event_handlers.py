...
from ui.cv_debug_windows import CVBallDebugWindow
...
class UIEventHandlersController:
    ...
    def handle_ball_button(self):
        ...
        if bool(getattr(host, "cv_ball_enabled", False)):
            host.cv_ball_enabled = False
            try:
                host.btn_CVBall.setStyleSheet(
                    """
    ...
    def handle_cv_ball_button(self):
        ...
        if bool(getattr(host, "ai_vision_enabled", False)):
        ...
        host.cv_ball_enabled = not bool(getattr(host, "cv_ball_enabled", False))
        ...
        state = "ON" if host.cv_ball_enabled else "OFF"
        print(f"[CV] CV Ball tracking {state}.")

        if host.cv_ball_enabled:
            host._cv_update_count = 0
            ...
            try:
                if host.cv_debug_window is None:
                    host.cv_debug_window = CVBallDebugWindow(
                        radial_checked=bool(getattr(host.ball_tracker, "cv_radial_gate_enabled", False)),
                        on_radial_toggle=host._on_cv_debug_radial_gate_changed,
                    )
                else:
                    try:
                        host.cv_debug_window.set_radial_gate_checked(
                            bool(getattr(host.ball_tracker, "cv_radial_gate_enabled", False))
                        )
                    except Exception:
                        pass
                host.cv_debug_window.move(host.x() + host.width() + 20, host.y() + 40)
                host.cv_debug_window.show()
                host.cv_debug_window.raise_()
                host._position_cv_hist_window()
            except Exception:
                pass
            ...
            try:
                if host.cv_debug_window is not None:
                    host.cv_debug_window.hide()
            except Exception:
                pass
            # Button style → cyan
            try:
    ...
    def handle_yolo_vision_button(self):
        ...
        if bool(getattr(host, "cv_ball_enabled", False)):
            ...
            try:
                ...
                pass
            try:
                if host.cv_hist_window is not None:
    ...
    def handle_ai_vision_button(self):
        ...
        if host.ai_one_shot_mode:
            if not host.ai_vision_enabled:
                ...
                host.ai_vision_enabled = True
                host._ai_one_shot_pending = True
                host._gpt_snapshot_count = int(getattr(host, "_gpt_snapshot_count", 0) or 0) + 1
                ...
                print(f"[GPT] GPT Vision SNAPSHOT armed (one request). #{host._gpt_snapshot_count}")
        ...
        host.ai_vision_enabled = not host.ai_vision_enabled
        state = "ON" if host.ai_vision_enabled else "OFF"
