...
python3 Test/DigitalTwin/scripts/digitaltwin_dev_server.py --host 127.0.0.1 --port 8766 --pi-host 192.168.0.32 --notify
```

2. Verify the page is reachable (`HTTP 200` expected):
```bash
curl -I http://127.0.0.1:8766/Test/DigitalTwin/pages/freenove_robotdog_3d_render.html
```

3. Open the page in Safari:
```bash
open -a Safari 'http://127.0.0.1:8766/Test/DigitalTwin/pages/freenove_robotdog_3d_render.html'
```
...
```bash
cat Test/DigitalTwin/docs/HIL_Stability_Gates.md
```
2. Run gait + HIL gate test:
```bash
python3 Test/DigitalTwin/scripts/test_gait_walk_playwright.py --headless
```
Expected artifact:
...
- Timestamped measurements saved under `Test/DigitalTwin/resources/specs/`.
...
Printable version:
- `Test/DigitalTwin/docs/MEASUREMENT_BENCH_SHEET.md`
...
3. Open files ready for updates:
  - `Test/DigitalTwin/resources/specs/robot_specs_template.json`
...
```bash
python3 Test/DigitalTwin/scripts/run_phase_verification.py --headless --only-phase B
python3 Test/DigitalTwin/scripts/run_phase_verification.py --headless --only-phase C
```
...
```bash
python3 Test/DigitalTwin/scripts/smoke_servo_sweep_playwright.py --headless
python3 Test/DigitalTwin/scripts/test_posture_modes_playwright.py --headless
python3 Test/DigitalTwin/scripts/test_gait_walk_playwright.py --headless
python3 Test/DigitalTwin/scripts/test_imu_feedback_playwright.py --headless
python3 Test/DigitalTwin/scripts/test_mujoco_ui_gate_playwright.py --headless
```

4. Run maximum-automation phased verification (recommended)
```bash
python3 Test/DigitalTwin/scripts/run_phase_verification.py --headless
```
...
```bash
cp Test/DigitalTwin/resources/line_messaging.env.example /tmp/line_messaging.env

# Fill token + recipient in /tmp/line_messaging.env, then:
python3 Test/DigitalTwin/scripts/line_messaging_api.py --env-file /tmp/line_messaging.env --validate-only
python3 Test/DigitalTwin/scripts/line_messaging_api.py --env-file /tmp/line_messaging.env --text "DigitalTwin checks PASS"
```

6. Optional: apply P1 calibration update and verify C/D in one step
```bash
python3 Test/DigitalTwin/scripts/p1_calibration_update.py \
...
```bash
python3 Test/DigitalTwin/scripts/p2_sim_estimate_and_update.py --headless --apply --verify
```
...
- The canonical workspace paths:
  - `Test/DigitalTwin/pages/`
  - `Test/DigitalTwin/scripts/`
  - `Test/DigitalTwin/resources/specs/`
...
- 2026-02-26 09:32 v1.24 - Added printable bench-sheet reference for first measurement session and linked to `Test/DigitalTwin/docs/MEASUREMENT_BENCH_SHEET.md`.
...
- 2026-02-26 08:33 v1.3 - Finalized folder reorganization to a single workspace `Code/Test/DigitalTwin`; updated command and page paths accordingly.
