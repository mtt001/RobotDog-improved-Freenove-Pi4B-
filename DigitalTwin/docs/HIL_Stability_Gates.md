<!--
File: DigitalTwin/docs/HIL_Stability_Gates.md
Description:
  Pre-MuJoCo HIL stability-gate design for the Digital Twin Three.js robot dog page.
Usage:
  1) Start local server: `python3 DigitalTwin/scripts/digitaltwin_dev_server.py --host 127.0.0.1 --port 8766`
  2) Verify reachability: `curl -I http://127.0.0.1:8766/DigitalTwin/pages/freenove_robotdog_3d_render.html` (expect `HTTP/1.0 200 OK` or `HTTP/1.1 200 OK`).
  3) Launch UI in Safari: `open -a Safari 'http://127.0.0.1:8766/DigitalTwin/pages/freenove_robotdog_3d_render.html'`
...
- Confirm path includes `/DigitalTwin/pages/freenove_robotdog_3d_render.html` exactly.
3. If Safari shows stale UI:
- Hard refresh and append cache buster, example:
  - `http://127.0.0.1:8766/DigitalTwin/pages/freenove_robotdog_3d_render.html?ts=20260226_2123`
