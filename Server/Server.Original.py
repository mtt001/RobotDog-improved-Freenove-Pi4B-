...
class Server:
    ...
    def turn_on_server(self):
        ...
        # Port 5001 for instruction sending and receiving
        self.server_socket1 = socket.socket()
        self.server_socket1.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEPORT, 1)
        self.server_socket1.bind((HOST, 5001))
        self.server_socket1.listen(1)

        print(f"Server bound on {HOST}")
    ...
    def battery_reminder(self):
        print(f"Battery voltage: {max(self.battery_voltage):.2f} V") 
        if max(self.battery_voltage) < 6.1:
            self.turn_off_server()
